...
public abstract class AbstractBatchIndexTask extends AbstractTask
{
  ...
  /**
   * Creates a predicate that allows only input rows which (a) are non-null and
  ...
  )
  {
    if (segmentsToWaitFor.isEmpty()) {
      ...
      return true;
    } else if (waitTimeoutMillis < 0) {
      log.warn("Not waiting for segment availability as waitTimeout[%s] is less than zero.", waitTimeoutMillis);
      return false;
    }
    ...
    try (
      ...
      segmentAvailabilityConfirmationCompleted = doneSignal.await(waitTimeoutMillis, TimeUnit.MILLISECONDS);
