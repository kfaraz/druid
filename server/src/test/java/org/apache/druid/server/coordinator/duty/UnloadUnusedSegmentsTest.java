...
public class UnloadUnusedSegmentsTest
{
  ...
  public void test_unloadUnusedSegmentsFromAllServers()
  {
    ...
    final DataSegment segmentWiki = null;
    final DataSegment segmentKoala = null;
    final DataSegment broadcastSegment = null;
    final DataSegment realtimeSegment = null;
    ...
    Assert.assertEquals(2L, stats.getSegmentStat(Stats.Segments.UNNEEDED, DruidServer.DEFAULT_TIER, segmentWiki.getDataSource()));
    Assert.assertEquals(1L, stats.getSegmentStat(Stats.Segments.UNNEEDED, "tier2", segmentWiki.getDataSource()));

    Assert.assertEquals(3L, stats.getSegmentStat(Stats.Segments.UNNEEDED, DruidServer.DEFAULT_TIER, DS.BROADCAST));
    Assert.assertEquals(1L, stats.getSegmentStat(Stats.Segments.UNNEEDED, "tier2", DS.BROADCAST));
  }

  private void setupRuleManager()
  {
    final LoadRule loadOnT1T2 = Load.on(DruidServer.DEFAULT_TIER, 1).andOn("tier2", 1).forever();
    ...
    databaseRuleManager.overrideRule(DS.BROADCAST, Collections.singletonList(Broadcast.forever()), null);
  }
